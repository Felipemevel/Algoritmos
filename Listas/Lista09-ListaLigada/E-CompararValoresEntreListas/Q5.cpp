...
public:
    ...
    void move_min_to_front() {
        ll_int_node *current = this->head;
        ll_int_node *temp1 = nullptr;
        ll_int_node *temp2 = nullptr;
    ...
    unsigned int contar_comuns(ll_int &other) {
        int count = 0;
        ...
        while (current != nullptr) {
            ll_int_node *compare = other.head;
...
int main() {
    ...
    for (int i = 0; i < vetor2->size(); i++) {
        std::cout << vetor2-> get_at(i) << " ";
    }
    std::cout << "]" << '\n';

    std::cout << "Valores comuns: "<< vetor->contar_comuns(*vetor2) << '\n';
